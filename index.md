...
- **Python 補助コード群**  
  *Python, PyTorch, Matplotlib, NumPy*  
  ...
  修士での研究や他の
  以下の通りです。  
  - /torchhelper/ → PyTorch を用いたコード生成のためのお助けライブラリ。全く異なるデータを使う場合でも使い回せる部分をモジュール化したコード群。
  - /music/ 音楽信号と和音を扱うためのユーティリティー (WAVEフォーマット Reader/Writer, ピッチ、コードネーム解析器等)
- **英語学習サイト解答収集ツール**  
  *C#, XML*  
  [source](https://github.com/03hcl/AlcNetAcademy)  
  ![(解答例)](/imgs/alc.png)
  ある英語学習サイトの解答データからスクレイピングを行って、  
  サイトで使われている解答データの整形式でないXMLに対して、個別に対応できるシリアライザー/デシリアライザーの作成に時間を使いました。
- **Arduinoを使ったデレステ譜面自動プレイ**  
  *Arduino, 電子工作, C#*  
  ...
  譜面データ形式は独自フォーマットのものを作成し[(例)](https://twitter.com/03hcl/status/1082610362153746433)、譜面データから Arduino で読み取るCSVファイルに変換するコンバーターについては C# で作成しました。  
...
- **音ゲー**
  現在ではサービスが終了しているのですが、BeatStream というアーケードの音ゲーをよくやっていました。
